...
type
  ...
  TObjectTagData=packed record {$region -fold}
    name              : AnsiString;
    obj_tag_data_ptr  : pointer;
    kind_of_object_tag: TObjectTag;
    is_selected       : boolean;
    is_focused        : boolean;
  end; {$endregion}
  PObjectTagData=^TObjectTagData;
  ...
  TSceneTree    =class         {$region -fold}
    public
      ...
      {Object tags}
      obj_tags_arr            : array of array of TObjectTagData;
     ...
     {procedure   MovSceneMT             (      rct_dst          :TPtRect;
      ...
      procedure   AddObjTag              (      node_with_data   :TTreeNode;
                                          const ot               :TObjectTag);        inline;   {$ifdef Linux}[local];{$endif}
...
procedure   TSceneTree.AddObjTag             (node_with_data:TTreeNode; const ot:TObjectTag);                                                                                              inline; {$ifdef Linux}[local];{$endif} {$region -fold}
var
  i,obj_ind,sel_obj_tags_arr_length: integer;
begin
  obj_ind:=PNodeData(node_with_data.Data)^.g_ind;
  SetLength(obj_tags_arr[obj_ind],Length(obj_tags_arr[obj_ind])+1);
  obj_tags_arr[obj_ind][Length(obj_tags_arr[obj_ind])-1].kind_of_object_tag:=ot;
  sel_obj_tags_arr_length:=0;
  for i:=0 to Length(obj_tags_arr)-1 do
    sel_obj_tags_arr_length+=Length(obj_tags_arr[i]);
...
begin
  ...
  case obj_var.obj_arr[g_ind].koo of
    kooBkgnd: node_with_data.ImageIndex:=06;
    kooRGrid: node_with_data.ImageIndex:=10;
    kooSGrid: node_with_data.ImageIndex:=09;
    kooGroup: node_with_data.ImageIndex:=11;
    kooTlMap: node_with_data.ImageIndex:=08;
    kooActor: node_with_data.ImageIndex:=12;
    kooPProc: node_with_data.ImageIndex:=07;
