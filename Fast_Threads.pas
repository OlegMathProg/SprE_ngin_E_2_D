...
uses
  ...
  Classes, SysUtils, MTProcs, PasMP, {Fast_Scene_Tree,} Fast_Graphics, Fast_UI;
...
begin
   ...
   //00: Empty;
     ...
     02: PPAlphaBlend(bmp_dst_ptr,bmp_dst_width,rct_dst,                color_info.pix_col,color_info.alpha1                  );
     03: PPAdditive  (bmp_dst_ptr,bmp_dst_width,rct_dst,                color_info.pix_col                                    );
        ...
        with sprite_arr [(images_inds_arr_ptr+i)^],fast_image_data,fast_image_proc_var do
          begin
            ...
            SdrProc[sdr_proc_ind](Trunc(x0+(item_rct.width -bmp_ftimg_width_origin                     *pt)/2),
                                  Trunc(y0+(item_rct.height-fast_image_data_arr[index].frame_height_int*pt)/2-fast_image_data_arr[index].rct_src_mrg.top*pt),
                                  @fast_image_data,
                                  index,
                                  block_count);
            fast_image_data_arr[index].scl_mul           :=PtPosF(1.0,1.0);
