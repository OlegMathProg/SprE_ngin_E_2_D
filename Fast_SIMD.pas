...
{This file contains some SIMD routines}
...
// SSE2:
procedure ClrAlphaByte128 (var      arr,       msk1:T4ArrL                      );          assembler; inline;
// AVX2:
procedure ClrAlphaByte256 (var      arr,       msk1:T8ArrL                      );          assembler; inline;

{Non-zero items count}
// replaces Fast_Graphics.ArrNzItCnt:
// SSE2:
function  NZByteCnt128    (const    src,{msk0,}msk1:TRGBA04                     ): byte;    assembler; inline;
// AVX2:
function  NZByteCnt256    (const    src            :TRGBA08                     ): word;    assembler; inline;
// AVX512:
function  NZByteCnt512    (const    src            :TRGBA16                     ): word;    assembler; inline; experimental;
...
procedure ScalePixInfo256A(var      arr0           :T4ArrW;
                           ...
                           constref c3             :double=1.0                  );          assembler; inline;
procedure ScalePixInfo256B(var      arr0           :T8ArrW;
                           var      arr1,arr2      :T8ArrL;
                           var      arr3           :T8ArrS;
                           constref c0,c1,c2       :single;
                           constref c3             :single=1.0                  );          assembler; inline; deprecated;
...
procedure ScalePixInfo256D(var      arr0           :T2_4ArrL;
                           const    arr1           :T4ArrD;
                           constref c0,c1          :double                      );          assembler; inline;
procedure ScalePixInfo256E(var      arr0           :T2_4ArrL;
                           const    arr1           :T4ArrD;
                           constref c0             :double                      );          assembler; inline;
...
procedure ScalePix128B    (const    msk0           :TRGBA04;
                           var      dst,src        :dword;
                           const    s0,s1          :dword;
                           const    arr_width      :dword                       );          assembler; inline;
...
procedure ScalePix128E    (const    msk0           :TRGBA04;
                           ...
                           const    src            :dword;
                           const    s0,s1          :dword;
                           const    b              :boolean                     );          assembler; inline; deprecated;
procedure ScalePix128F    (const    msk0           :TRGBA04;
                           var      dst            :qword;
                           const    src            :dword;
                           const    s0,s1,s2,s3    :dword;
                           const    b              :boolean;
                           const    arr_width      :dword                       );          assembler; inline; deprecated;
// AVX2:
procedure ScalePix256     (const    msk0,msk1      :TRGBA08;
                           var      dst,src        :dword;
                           const    s0,s1,s2,s3    :dword;
                           const    arr_width      :dword                       );          assembler; inline; unimplemented;
...
procedure Point3x3128     (const    msk0           :TRGBA04;
                           ...
                           const    src            :dword;
                           const    s0,s1          :dword;
                           const    arr_width      :dword                       );          assembler; inline; experimental;
...
{Convert 4 double to 4 integer: AVX2}
procedure DoubleToInt256  (const src      :T4ArrD; var dst:T4ArrI                                                                                                           );       assembler; inline; nostackframe;                {$region -fold}
asm
  vmovupd    ymm0, [src]
...
{Convert 8 single to 8 integer: AVX2}
procedure SingleToInt256  (const src      :T8ArrS; var dst:T8ArrI                                                                                                           );       assembler; inline; nostackframe;                {$region -fold}
asm
  vmovups    ymm0, [src]
...
// SSE2:
procedure ClrAlphaByte128 (var   arr ,msk1:T4ArrL                                                                                                                           );       assembler; inline; nostackframe;                {$region -fold}
asm
  movdqu xmm0 ,[arr]
  pand   xmm0 ,[msk1]
  movdqu [arr],xmm0
...
// AVX2:
procedure ClrAlphaByte256 (var   arr ,msk1:T8ArrL                                                                                                                           );       assembler; inline; nostackframe;                {$region -fold}
asm
  vmovdqu    ymm0 ,[arr]
...
// replaces Fast_Graphics.ArrNzItCnt:
// SSE2:
function  NZByteCnt128    (const src ,msk1:TRGBA04                                                                                                                          ): byte; assembler; inline; nostackframe;                {$region -fold}
asm
  pxor    xmm3    ,xmm3
  movdqu  xmm0    ,[src]
  pcmpeqb xmm0    ,xmm3
...
// AVX2:
function  NZByteCnt256    (const src      :TRGBA08                                                                                                                          ): word; assembler; inline; nostackframe;                {$region -fold}
asm
...
// AVX512:
function  NZByteCnt512    (const src      :TRGBA16                                                                                                                          ): word; assembler; inline; nostackframe; experimental;  {$region -fold}
asm
  {
...
{Prepare pixel scale info: AVX2}
// used in Fast_Graphics:
procedure ScalePixInfo256A(var arr0:T4ArrW;   var   arr1,arr2:T4ArrL; var arr3:T4ArrD;                                      constref c0,c1,c2:double; constref c3:double=1.0);       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePixInfo256B(var arr0:T8ArrW;   var   arr1,arr2:T8ArrL; var arr3:T8ArrS;                                      constref c0,c1,c2:single; constref c3:single=1.0);       assembler; inline;               deprecated;    {$region -fold}
asm
...
procedure ScalePixInfo256C(                   var   arr1,arr2:T4ArrL; var arr3:T4ArrD; constref msk:T4ArrD; const pix:word; constref c0,c1,c2:double; constref c3:double=1.0);       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePixInfo256D(var arr0:T2_4ArrL; const arr1     :T4ArrD;                                                       constref c0,c1   :double                        );       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePixInfo256E(var arr0:T2_4ArrL; const arr1     :T4ArrD;                                                       constref c0      :double                        );       assembler; inline;                              {$region -fold}
asm
...
// replaces Fast_Graphics.ScalePix:
// SSE2:
procedure ScalePix128A    (const msk0     :TRGBA04; var dst,src:dword;                  const s0,s1      :dword; const b:boolean                                            );       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePix128B    (const msk0     :TRGBA04; var dst,src:dword;                  const s0,s1      :dword;                  const arr_width:dword                     );       assembler; inline;                              {$region -fold}
asm
...
// replaces Fast_Graphics.AlphaBlend4:
// SSE2:
procedure ScalePix128C    (const msk0,msk1:TRGBA04; var dst,src:dword;                  const s0,s1,s2   :dword; const b:boolean                                            );       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePix128D    (const msk0,msk1:TRGBA04; var dst,src:dword;                  const s0,s1,s2   :dword;                  const arr_width:dword                     );       assembler; inline;                              {$region -fold}
asm
...
procedure ScalePix128E    (const msk0     :TRGBA04; var dst    :qword; const src:dword; const s0,s1      :dword; const b:boolean                                            );       assembler; inline;               deprecated;    {$region -fold}
asm
...
procedure ScalePix128F    (const msk0     :TRGBA04; var dst    :qword; const src:dword; const s0,s1,s2,s3:dword; const b:boolean; const arr_width:dword                     );       assembler; inline;               deprecated;    {$region -fold}
asm
...
// AVX2:
procedure ScalePix256     (const msk0,msk1:TRGBA08; var dst,src:dword;                  const s0,s1,s2,s3:dword;                  const arr_width:dword                     );       assembler; inline;               unimplemented; {$region -fold}
asm
  {TODO}
end; {$endregion}
...
// replaces Fast_Graphics.Point:
procedure Point3x3128     (const msk0     :TRGBA04; var dst    :dword; const src:dword; const s0,s1      :dword;                  const arr_width:dword                     );       assembler; inline;               experimental;  {$region -fold}
asm
