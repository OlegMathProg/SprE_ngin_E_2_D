...
{AlphaBlend}
// SSE2:
procedure AlphaBlend128A  (         pixel_ptr      :PInteger;
                           const    src_pix        :T4ArrL;
                           const    alpha_         :T4ArrL                      );          assembler;
procedure AlphaBlend128B  (         pixel_ptr      :PInteger;
                           const    src_pix        :T4ArrL;
                           const    alpha_         :T4ArrL                      );          assembler;
procedure AlphaBlend128C  (         pixel_ptr      :PInteger;
                           const    src_pix        :T4ArrL;
                           const    alpha_         :T4ArrL                      );          assembler;
procedure AlphaBlend128D  (         pixel_ptr      :PInteger;
                           const    src_pix        :T4ArrL;
                           const    alpha_         :T4ArrL                      );          assembler;
procedure AlphaBlend128E  (         pixel_ptr      :PInteger;
                           const    src_pix        :T4ArrL;
                           const    alpha_         :T4ArrL                      );          assembler;
// AVX2:
procedure AlphaBlend256A  (         pixel_ptr      :PInteger;
                           const    src_pix        :T8ArrL;
                           const    alpha_         :T8ArrL                      );          assembler;
procedure AlphaBlend256B  (         pixel_ptr      :PInteger;
                           const    src_pix        :T8ArrL;
                           const    alpha_         :T8ArrL                      );          assembler;
procedure AlphaBlend256C  (         pixel_ptr      :PInteger;
                           const    src_pix        :T8ArrL;
                           const    alpha_         :T8ArrL                      );          assembler;
procedure AlphaBlend256D  (         pixel_ptr      :PInteger;
                           const    src_pix        :T8ArrL;
                           const    alpha_         :T8ArrL                      );          assembler;
procedure AlphaBlend256E  (         pixel_ptr      :PInteger;
                           const    src_pix        :T8ArrL;
                           const    alpha_         :T8ArrL                      );          assembler; experimental;
...
{Check if POPCNT is supported}
function  HasPOPCNT: boolean; {$region -fold}
var
  a,b,c,d: dword;
begin
  a:=1;
...
{AlphaBlend}
// SSE2:
procedure AlphaBlend128A  (pixel_ptr:PInteger; const src_pix:T4ArrL; const alpha_:T4ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm

  pxor      xmm5                 ,xmm5

  {load (pixel_ptr+0)^,..., (pixel_ptr+3)^}
 {mov       rax                  ,pixel_ptr // load pointer
  movdqu    xmm0                 ,[rax]}
  movdqu    xmm0                 ,dqword ptr[pixel_ptr]

  {copy (pixel_ptr+0)^,..., (pixel_ptr+3)^}
  movdqu    xmm1                 ,xmm0

  {in each of 16-bit part will be 16-bit extended color component of (pixel_ptr+0)^ and (pixel_ptr+1)^}
  punpcklbw xmm0                 ,xmm5

  {in each of 16-bit part will be 16-bit extended color component of (pixel_ptr+2)^ and (pixel_ptr+3)^}
  punpckhbw xmm1                 ,xmm5
  ...
  { (src_pix-pixel_ptr^)*alpha_}
  pmullw    xmm2                 ,xmm4
  pmullw    xmm3                 ,xmm4

  {((src_pix-pixel_ptr^)*alpha_)>>8}
  psraw     xmm2                 ,8
  psraw     xmm3                 ,8

  {((src_pix-pixel_ptr^)*alpha_)>>8+pixel_ptr^}
  paddw     xmm2                 ,xmm0
  paddw     xmm3                 ,xmm1
  ...
  {store result}
 {movdqu    [rax]                ,xmm2}
  movdqu    dqword ptr[pixel_ptr],xmm2

end; {$endregion}
procedure AlphaBlend128B  (pixel_ptr:PInteger; const src_pix:T4ArrL; const alpha_:T4ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
  {Byte(alpha_)=128+k;
   dst+((src-dst)*Byte(alpha_)>>8)=
   dst+((src-dst)*128         >>8)
      +((src-dst)*k           >>8)=
   dst+ (src-dst)/2+
       ((src-dst)*k           >>8)=
...
procedure AlphaBlend128C  (pixel_ptr:PInteger; const src_pix:T4ArrL; const alpha_:T4ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
 ...
 {mov       rax                  ,pixel_ptr // load pointer
  ...
  { (pixel_ptr^-src_pix)*alpha_}
  pmullw    xmm0                 ,xmm4
  pmullw    xmm1                 ,xmm4

  {((pixel_ptr^-src_pix)*alpha_)>>8}
  psraw     xmm0                 ,8
  psraw     xmm1                 ,8

  {((pixel_ptr^-src_pix)*alpha_)>>8+src_pix}
  paddw     xmm0                 ,xmm2
  paddw     xmm1                 ,xmm3
  ...
  {store result}
 {movdqu    [rax]                ,xmm0}
  movdqu    dqword ptr[pixel_ptr],xmm0

end; {$endregion}
procedure AlphaBlend128D  (pixel_ptr:PInteger; const src_pix:T4ArrL; const alpha_:T4ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
  {Byte(alpha_)=128+k;
   src+((dst-src)*Byte(alpha_)>>8)=
   src+((dst-src)*128         >>8)
      +((dst-src)*k           >>8)=
   src+ (dst-src)/2+
       ((dst-src)*k           >>8)=
...
procedure AlphaBlend128E  (pixel_ptr:PInteger; const src_pix:T4ArrL; const alpha_:T4ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
  ...
  {load pixel_ptr^}
  movdqu    xmm1                  ,xmmword ptr[pixel_ptr]
  movdqa    xmm3                  ,xmm1
  punpcklbw xmm1                  ,xmm7
  punpckhbw xmm3                  ,xmm7
  movdqa    xmm6                  ,xmm1
  ...
  {store pixel_ptr^}
  movdqu    xmmword ptr[pixel_ptr],xmm0

end; {$endregion}
// AVX2:
procedure AlphaBlend256A  (pixel_ptr:PInteger; const src_pix:T8ArrL; const alpha_:T8ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm

  vpxor      ymm5                  ,ymm5                  ,ymm5

  {load (pixel_ptr+0)^,..., (pixel_ptr+7)^}
  vmovdqu    ymm0                  ,ymmword ptr[pixel_ptr]
  vmovdqa    ymm1                  ,ymm0
  ...
  {store result}
  vmovdqu    ymmword ptr[pixel_ptr],ymm2

  vzeroupper

end; {$endregion}
procedure AlphaBlend256B  (pixel_ptr:PInteger; const src_pix:T8ArrL; const alpha_:T8ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm

  {same formula for alphablend as in AlphaBlend128B}
...
procedure AlphaBlend256C  (pixel_ptr:PInteger; const src_pix:T8ArrL; const alpha_:T8ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
...
procedure AlphaBlend256D  (pixel_ptr:PInteger; const src_pix:T8ArrL; const alpha_:T8ArrL                                                                                    );       assembler; nostackframe;                {$region -fold}
asm
  // TODO
end; {$endregion}
procedure AlphaBlend256E  (pixel_ptr:PInteger; const src_pix:T8ArrL; const alpha_:T8ArrL                                                                                    );       assembler; nostackframe; experimental;  {$region -fold}
asm

  vpxor      ymm5                  ,ymm5                  ,ymm5

  vmovdqu    ymm0                  ,ymmword ptr[pixel_ptr]      // dst
  vmovdqu    ymm1                  ,[src_pix]                   // src
  vmovdqu    ymm4                  ,[alpha_]                    // alpha

  vmovdqu    ymm2                  ,ymm0
  vmovdqu    ymm3                  ,ymm1

  vpunpcklbw ymm0                  ,ymm0                  ,ymm5 // low 8 pixels dst
  vpunpckhbw ymm2                  ,ymm2                  ,ymm5 // high 8 pixels dst
  vpunpcklbw ymm1                  ,ymm1                  ,ymm5 // low 8 pixels src
  vpunpckhbw ymm3                  ,ymm3                  ,ymm5 // high 8 pixels src

  vmovdqu    ymm6                  ,ymm4
  vpunpcklbw ymm4                  ,ymm4                  ,ymm5 // low alpha
  vpunpckhbw ymm6                  ,ymm6                  ,ymm5 // high alpha

  vpsubw     ymm1                  ,ymm1                  ,ymm0
  vpsubw     ymm3                  ,ymm3                  ,ymm2

  vpmullw    ymm1                  ,ymm1                  ,ymm4
  vpmullw    ymm3                  ,ymm3                  ,ymm6
  ...
  vmovdqu    ymmword ptr[pixel_ptr],ymm0
